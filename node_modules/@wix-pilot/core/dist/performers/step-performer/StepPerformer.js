...
const extractCodeBlock_1 = require("../../common/extract/extractCodeBlock");
...
class StepPerformer {
    ...
    async generateCode(currentStep, previousSteps, screenCapture) {
        ...
        if (this.cacheHandler.isCacheInUse() && cacheKey && snapshotHashes) {
            ...
            if (cachedValues) {
                const matchingEntry = this.cacheHandler.findMatchingCacheEntry(cachedValues, snapshotHashes);
                if (matchingEntry) {
        ...
        if (this.cacheHandler.isCacheInUse() && cacheKey) {
            ...
            this.cacheHandler.addToTemporaryCache(cacheKey, cacheValue, snapshotHashes);
        }
        return code;
    }
    async perform(step, previous = [], screenCapture, maxAttempts = 2) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                const screenCaptureResult = attempt == 1
                    ? screenCapture
                    : await this.screenCapturer.capture(true);
                const code = await this.generateCode(step, previous, screenCaptureResult);
                lastCode = code;
                if (!code) {
